...
void Si4703_Breakout::setChannel(int channel)
{
  ...
  readRegisters();
  si4703_registers[CHANNEL] &= ~(1<<TUNE); //Clear the tune after a tune has completed_text
  updateRegisters();
...
{ 
  ...
  boolean completed_main[] = {false, false, false, false};
  
  boolean completed_text[] = {false, false, false, false,
						      false, false, false, false,
						      false, false, false, false,
						      false, false, false, false };
  int completedCount = 0;
  boolean changeDetected = false;
  while( !_cancelRDS && completedCount < 4 && millis() < endTime) {
	readRegisters();
	if(si4703_registers[STATUSRSSI] & (1<<RDSR))
	{
		/*
		sprintf(rds_regs, "%02X %02X %02X %02X %02X %02X %02X %02X\n",
			(si4703_registers[RDSA] & 0xFF00) >> 8,
			 si4703_registers[RDSA] & 0x00FF,
			(si4703_registers[RDSB] & 0xFF00) >> 8,
			 si4703_registers[RDSB] & 0x00FF,
			(si4703_registers[RDSC] & 0xFF00) >> 8,
			 si4703_registers[RDSC] & 0x00FF,
			(si4703_registers[RDSD] & 0xFF00) >> 8,
			 si4703_registers[RDSD] & 0x00FF
		);
		Serial.print(rds_regs);
		*/
	  ...
	  int index = b & 0x03;
	  if ( ((b & 0xF000) == 0x0000) ) //&& !completed_main[index] )  
	  {
		//index &= 0x03;
		completed_main[index] = true;
		completedCount ++;
		...
		if( !changeDetected &&
		    _rds_1[index * 2] != '\0' &&
			( buffer[index * 2] != _rds_1[index * 2] || 
			  buffer[index * 2 + 1] != _rds_1[index * 2 + 1] ) )
		{
		  changeDetected = true;
		  for( int i = 0; i < 4; i++ )
		  {
			if( i != index && completed_main[i] == true )
			{
		      completed_main[i] = false;
		      completedCount--;
			}
		  }
		}
	  ...
	  /*
	  else if ( ( (b & 0xF000) == 0x2000 ) && !completed_text[index] )
	  {
		completed_text[index] = true;
		completedCount ++;
      	...
      	char Dl = (si4703_registers[RDSD] & 0x00FF);
		buffer[9 + index * 4]     = Ch;
		buffer[9 + index * 4 + 1] = Cl;
		buffer[9 + index * 4 + 2] = Dh;
		buffer[9 + index * 4 + 3] = Dl;
		/*
		Serial.print(index, DEC);
		Serial.write(Ch);
		Serial.write(Cl);
  ...
  _cancelRDS = 0;
  if ( changeDetected || millis() >= endTime )
  {
	memset(buffer, 0, 8);
	memset(_rds_1, 0, 8);
	return;
...
{
	readRegisters(); //Read the current register set
	return (si4703_registers[STATUSRSSI] & 0xFF );
}

void Si4703_Breakout::cancelRDS()
{
	_cancelRDS = 1;
}
...
int Si4703_Breakout::seek(byte seekDirection){
  ...
  si4703_registers[POWERCFG] &= ~(1<<SEEK); //Clear the seek bit after seek has completed_text
  updateRegisters();
